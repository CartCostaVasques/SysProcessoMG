...
export function AppProvider({ children }) {
  ...
  const salvarCartorio = useCallback(async (d) => { try { const {error} = await supabase.from('cartorio').update(d).eq('id',1); if(error) throw error; setCartorio(prev => ({...prev, ...d})); addToast('Configurações salvas!','success'); } catch(e){ addToast(e.message,'error'); } }, []);
