...
function AbaCasamentos({ sb, addToast, usuarios, cartorio }) {
  ...
  const gerarOficioJuizPaz = async () => {
    ...
    try {
      ...
      const th = (text,w) => new TableCell({ width:{size:w,type:WidthType.DXA}, borders:brdAll, margins:mC, shading:{fill:'D9D9D9',type:ShadingType.CLEAR}, children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text,font:FONTE,size:20,bold:true,color:'000000'})]})] });
      ...
      const doc = new Document({
        ...
        sections:[{
          ...
          children:[
            ...
            pV(),
            p([t('CASAMENTOS AGENDADOS',true,true)], AlignmentType.CENTER, {before:0,after:120}),
            tabela,
            pV(),
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <select value={filtroAno} onChange={e => setFiltroAno(e.target.value ? parseInt(e.target.value) : '')}
          style={{ fontSize: 12, height: 32, padding: '0 8px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-surface-2)', color: 'var(--color-text)', cursor: 'pointer' }}>
          <option value="">Todos anos</option>
          {anosDisponiveis.map(a => <option key={a} value={a}>{a}</option>)}
        </select>
        <select value={filtroMes} onChange={e => setFiltroMes(e.target.value !== '' ? parseInt(e.target.value) : '')}
          style={{ fontSize: 12, height: 32, padding: '0 8px', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-surface-2)', color: 'var(--color-text)', cursor: 'pointer' }}>
