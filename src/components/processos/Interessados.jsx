...
export default function Interessados() {
  ...
  const saveEdit = async () => {
    await editInteressado(editingId, editRow);
    setEditingId(null);
  };

  const handleDelete = (i) => {
    if (window.confirm(`Remover "${i.nome}"?`)) deleteInteressado(i.id);
  };
    ...
    <div className="fade-in">
      ...
      <div className="table-wrapper">
        <table className="data-table" style={{ fontSize: 13 }}>
          ...
          <tbody>
            {lista.length === 0 && (
              <tr><td colSpan={7}><div className="empty-state"><div className="empty-state-icon">👤</div><div className="empty-state-text">Nenhum interessado cadastrado</div></div></td></tr>
            )}
